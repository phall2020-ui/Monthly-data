"""
Improved Waterfall Chart for Solar Loss Analysis
- Filter by Site, Type, Date Range
- View by Period (Monthly, Quarterly, YTD, Annual)
- Efficiency Loss instead of PR Loss
"""
    ...
    with tab:
        ...
        if date_range and len(date_range) == 2 and "_date" in df_filtered.columns:
            ...
            df_filtered = df_filtered[
                (df_filtered["_date"] >= pd.Timestamp(start)) &
                (df_filtered["_date"] <= pd.Timestamp(end))
            ]
        ...
        with view_col1:
            view_mode = st.selectbox(
                "Group by",
                ["Total (Summary)", "By Period", "By Site", "By Type"],
                key="wf_view_mode"
            )
        ...
        if view_mode == "Total (Summary)":
            ...
            with m3:
                var_pct = ((metrics['Actual'] - metrics['Budget']) / metrics['Budget'] * 100) if metrics['Budget'] else 0
            ...
            breakdown_df = pd.DataFrame([
                {"Component": "Weather Variance", "kWh": metrics['Weather Var'], 
                 "% of Budget": metrics['Weather Var']/metrics['Budget']*100 if metrics['Budget'] else 0},
                {"Component": "Availability Loss", "kWh": -metrics['Avail Loss'], 
                 "% of Budget": -metrics['Avail Loss']/metrics['Budget']*100 if metrics['Budget'] else 0},
                {"Component": "Other Losses", "kWh": -other_loss, 
                 "% of Budget": -other_loss/metrics['Budget']*100 if metrics['Budget'] else 0},
                {"Component": "Total Efficiency Loss", "kWh": -metrics['Efficiency Loss'], 
                 "% of Budget": -metrics['Efficiency Loss']/metrics['Budget']*100 if metrics['Budget'] else 0},
            ])
            ...
            if chart_option == "Stacked Bar (Budget vs Actual)":
                ...
                selected_group = st.selectbox(
                    f"Select {view_mode.replace('By ', '')}",
                    results_df[group_col].tolist(),
                    key="wf_select_group"
                )
                ...
                fig.update_layout(
                    title=f"Variance Trends by {view_mode.replace('By ', '')}",
