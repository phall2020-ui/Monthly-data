...
def render_calculations_tab_simple(tab, extractor):
    ...
    with tab:
        ...
        # Site filter
        site_col = colmap.get("site") or next(
            (c for c in df.columns if "site" in c.lower()), None
        )
        selected_sites = None
        ...
        type_col = next(
            (c for c in df.columns if any(x in c.lower() for x in ["type", "technology", "category", "class"])),
            None
        )
        selected_types = None
        ...
        weight_col = colmap.get("actual_gen") or next(
            (c for c in df_filtered.columns if "actual" in c.lower() and "gen" in c.lower()),
            None
        )
        ...
        if view_mode == "Summary (Total)":
            ...
            with chart_col1:
                chart_metric = st.selectbox(
                    "Metric to chart",
                    selected_kpis,
                    format_func=lambda x: KPI_DEFINITIONS.get(x, {}).get("display", x)
                )
